            ...
            for (LONG dir = -1; dir <= 1 && !resolved; dir += 2) {{
                ...
                for (DWORD k = 0; k < 32 && !resolved; k++) {{
                    if (pN[k] == 0x4Cu && pN[k+1u] == 0x8Bu &&
                        ...
                        /* Adjust: our SSN = neighbor SSN ∓ delta */
...
#endif
    /* RTL_USER_PROCESS_PARAMETERS is at pPeb+0x20 (x64) or pPeb+0x10 (x86).
     * OSBuildNumber is at pPeb+0x120 (x64) / pPeb+0xA4 (x86) on modern Windows. */
#if defined(_WIN64)
 ...
 *  Requires the stub section to be writable — VirtualProtect is used.
 * ========================================================================= */
#define {p}EGG_SIZE  8U
static const BYTE {p}EggPattern[{p}EGG_SIZE] = {{ {egg_bytes_c} }};

BOOL {p}HatchEggs(VOID) {{
    /* Locate our own PE image base via PEB */
    PPEB pPeb =
#if defined(_WIN64)
        (PPEB)__readgsqword(0x60);
#else
...
BOOL {p}PatchEtw(VOID) {{
    ...
    PVOID pTarget = {p}GetProcByHash(pNtdll, {djb2_hash('EtwEventWrite'):08X}U);
...
{junk}    mov eax, DWORD PTR [{p}SsnTable + {idx * 4}]  ; load SSN
    syscall
    ret
{fname} ENDP
...
{junk}    rdtsc                                   ; eax = TSC_low, edx = TSC_high
    ...
    mov  eax, DWORD PTR [{p}SsnTable + {idx * 4}]  ; eax = SSN
...
"""
    ...
    def _asm_stack_spoof_helper(self, p: str) -> str:
        return f"""\
; =========================================================================
; Stack spoofing helper (basic synthetic frame)
...
"""
    ...
    __asm__ __volatile__ (
        ...
        ::: "memory"
            ...
            hex_pairs = "".join(f"\\x{b:02x}" for b in egg_bytes_list)
            asm_body = f"""\
    __asm__ __volatile__ (
        "mov r10, rcx\\n"
        "mov eax, [{p}SsnTable + {idx * 4}]\\n"
